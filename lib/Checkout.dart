...
//import '/payment/init-payment.dart';
...
void onButtonTap(String selected,context) async {
  ...
  final user = jsonDecode(pref.getString('auth')??'{}')['user'][1];    
  final deposit = context.watch<DepositProvider>();
int payAmount = deposit.paymentAmount;
  String balanceType = deposit.paymentType;
    ...
    case 'bkash':
      /*bkashPayment(context,customerData);*/{
        ...
        if(res.statusCode==200){
          final bkashUrl=jsonDecode(res.body)['bkashURL'];
          Navigator.push(context, MaterialPageRoute(builder: (context)=>WebView(url:bkashUrl)));return;
       
        }
        Toast({'message':'Could not launch bkash checkout','success':false});
      //final url = ;
      }
      break;

    case 'sslcommerz':
      /*initiateSSLCPayment(customerData,context);*/{
      final res = await http.post(
        Uri.parse(ApiConstants.sslcPaymentInit),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(/*{
      ...
      if(res.statusCode==200) {
        ...
        final gatewayUrl = resData['url'];Navigator.push(context, MaterialPageRoute(builder: (context)=>WebView(url:gatewayUrl)));

        /*if (await canLaunchUrl(Uri.parse(gatewayUrl))) {
          await launchUrl(Uri.parse(gatewayUrl), mode: LaunchMode.externalApplication);
        } else {Toast({'message':'Could not launch $gatewayUrl','success':false},context);}
          Toast({'message':'Could not launch $gatewayUrl','success':false},context);*/
      }else {Toast({'message':'Payment initiation failed','success':false});}
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (_, index) {
                      return PaymentMethodTile(
                        logo: deposit.pgws[index]['icon'],
                        name: deposit.pgws[index]['name'],
                        selected: selected ?? '',
                        onTap: () {
                          selected = deposit.pgws[index]['name'].toString().replaceAll(' ', '_').toLowerCase();
                          //setState(() {});
                        },
                      );
                    },
                    separatorBuilder: (_, index) => const SizedBox(height: 10),
                    itemCount: deposit.pgws.length,
                  ),
                ],
              ),
            ),
